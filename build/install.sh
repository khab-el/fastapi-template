...
if [[ "$PYTHON_MAJOR_VERSION" -lt 3 ]] || [[ "$PYTHON_MINOR_VERSION" -lt 8 ]]; then
  echo "[install] Python version 3.11.0 or higher is required."
  exit 1
fi
...
Now you should access CLI script: $ core --help
Alternatively you can access CLI script via poetry run: $ poetry run core --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_EXAMPLE_COMPLETE=source_bash core)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_EXAMPLE_COMPLETE=source_zsh core)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_EXAMPLE_COMPLETE=source_fish core)' >> ~/.config/fish/completions/example.fish
EOF